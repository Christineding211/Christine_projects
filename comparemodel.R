...
#after compareing leave two model
#deaths ~ cases + icu
#deaths~ icu + weekly
...
#check the QQplot and VIF of the deaths ~ cases + icu
us_cases<- us_covid_train 
us_cases$predicted<- predict(mod_cases)
us_cases$residuals<- residuals(mod_cases)
View(us_cases)

qqnorm(us_cases$residuals, pch = 1, frame = FALSE)
qqline(us_cases$residuals, col = "steelblue", lwd = 2)

#Variance Inflation Factor (VIF): 
install.packages("car")
library(car)
vif_casesvalues <- vif(mod_cases)
print(vif_casesvalues)


us_icu<- us_covid_train 
us_icu$predicted<- predict(mod_icu)
us_icu$residuals<- residuals(mod_icu)
View(us_icu)

qqnorm(us_icu$residuals, pch = 1, frame = FALSE)
qqline(us_icu$residuals, col = "steelblue", lwd = 2)
